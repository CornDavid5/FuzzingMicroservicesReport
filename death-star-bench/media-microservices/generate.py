...
with open('interactions.txt', mode ='r') as file:
    ...
    for line in lines :
        ...
        if parts[0] == 'nginx-thrift':
            # api gateway
            label = 'URL: ' + additional_info[0] + '\nRPC Call: ' + additional_info[1] + '\nData Sent: ' + additional_info[2]
            interactions[parts[0]][parts[1]].append(label)
        elif len(additional_info) == 3:
            # database interactions
            label = 'Database: ' + additional_info[0] + '\nCollection: ' + additional_info[1] + '\nData Sent: ' + additional_info[2]
            interactions[parts[0]][parts[1]].append(label)
        elif additional_info[0] in ['get', 'set', 'del', 'add', 'inc']:
            # cache interactions
...
net = Network(heading='Interaction between services within Media Microservices', directed=True)
